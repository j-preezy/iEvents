...
@Composable
fun ArtEvents(navController: NavController) {
    var isLoading by remember { mutableStateOf(true) }
    var artList by remember { mutableStateOf(emptyList<Product>()) }
    var displayedArtCount by remember { mutableStateOf(5) }
                ...
                if (isLoading) {
                    ...
                    if (artList.isEmpty()) {
                        ...
                        LazyVerticalGrid(
                            ...
                            items(artList.take(displayedArtCount)) { product ->
...
suspend fun fetchArt(onSuccess: (List<Product>) -> Unit) {
    val firestore = Firebase.firestore
    val snapshot = firestore.collection("Events")
        .whereEqualTo("category", "Art") // Fetch events with category "Music"
        .get().await()
    val artList = snapshot.documents.mapNotNull { doc ->
        val art = doc.toObject<Product>()
        art?.id = doc.id
        art
    }
    onSuccess(artList)
