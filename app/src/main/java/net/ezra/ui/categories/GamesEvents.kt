...
@Composable
fun GamesEvents(navController: NavController) {
    var isLoading by remember { mutableStateOf(true) }
    var gamesList by remember { mutableStateOf(emptyList<Product>()) }
    var displayedGamesCount by remember { mutableStateOf(5) }
                ...
                if (isLoading) {
                    ...
                    if (gamesList.isEmpty()) {
                        ...
                        LazyVerticalGrid(
                            ...
                            items(gamesList.take(displayedGamesCount)) { product ->
...
suspend fun fetchGames(onSuccess: (List<Product>) -> Unit) {
    val firestore = Firebase.firestore
    val snapshot = firestore.collection("Events")
        .whereEqualTo("category", "Games") // Fetch events with category "Music"
        .get().await()
    val gamesList = snapshot.documents.mapNotNull { doc ->
        val games = doc.toObject<Product>()
        games?.id = doc.id
        games
    }
    onSuccess(gamesList)
